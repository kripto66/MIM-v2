...
export async function runDeclarations(r, ctx) {
  ...
  const loc1 = owner1.locataires[0]; // own1loc1 : mois courant attente + mois préc. retard
  const loc2 = owner1.locataires[1]; // own1loc2 : mois courant paye
  const loc21 = owner2.locataires[0]; // own2loc1 : mois courant attente + mois préc. retard
  ...
  const loyerOf = async (logementId) => {
    const { data } = await service.from('logements').select('loyer_mensuel').eq('id', logementId).single();
    return data?.loyer_mensuel;
  };
  ...
  await r.section('visibilité des moyens par le locataire', async () => {
    ...
    const me = await api('/auth/me', { jar: t1 });
    const myUid = me.data.user.id;
  ...
  await r.section('déclaration du locataire', async () => {
    const ownerJar = await loginOwner(owner1);
    const t1 = await loginTenant(loc1.username);
    ...
    const retard = await payOf(loc1.id, seed.prev);
    if (!retard) {
      r.fail(S, 'paiement retard seed présent', 'introuvable');
      return;
    }

    const noMoyen = await api(`/locataire/paiements/${retard.id}/declarer`, { method: 'POST', jar: t1, body: {} });
    ...
    const otherPay = await payOf(loc2.id, seed.month);
    const crossLoc = await api(`/locataire/paiements/${otherPay.id}/declarer`, { method: 'POST', jar: t1, body: { moyen_paiement_id: waveMoyen.id } });
    ...
    const foreignMoyen = await api('/moyens-paiement', { method: 'POST', jar: await loginOwner(owner2), body: { type: 'wave', numero: '+22199000000' } });
    const crossMoyen = await api(`/locataire/paiements/${retard.id}/declarer`, { method: 'POST', jar: t1, body: { moyen_paiement_id: foreignMoyen.data.data.id } });
    ...
    const dec = await api(`/locataire/paiements/${retard.id}/declarer`, {
      method: 'POST', jar: t1,
    ...
    const twice = await api(`/locataire/paiements/${retard.id}/declarer`, { method: 'POST', jar: t1, body: { moyen_paiement_id: waveMoyen.id } });
    ...
    const onPaye = await api(`/locataire/paiements/${otherPay.id}/declarer`, { method: 'POST', jar: t1, body: { moyen_paiement_id: waveMoyen.id } });
    if (onPaye.status === 404) r.pass(S, 'paiement d\'un autre locataire non déclarable (404)');
    else r.fail(S, 'paiement d\'un autre locataire non déclarable (404)', `statut ${onPaye.status}`);

    // Déclaration du mois courant (pour le flux « validation -> nouvelle échéance »).
    const courant = await payOf(loc1.id, seed.month);
    const decCourant = await api(`/locataire/paiements/${courant.id}/declarer`, { method: 'POST', jar: t1, body: { moyen_paiement_id: waveMoyen.id } });
  ...
  await r.section('paiements en attente (propriétaire)', async () => {
    ...
    if (items.length !== 2 || !items.every((i) => String(i.locataire_id) === String(loc1.id))) r.fail(S, '2 déclarations visibles (own1loc1)', `len=${items.length} ${items.map((i) => i.locataire_id).join(',')}`);
    else r.pass(S, '2 déclarations visibles (own1loc1)');
  ...
  await r.section('validation du propriétaire', async () => {
    ...
    const retard = await payOf(loc1.id, seed.prev);
    const payePay = await payOf(loc2.id, seed.month);

    const cross = await api(`/paiements-validation/${retard.id}/valider`, { method: 'POST', jar: otherJar });
    ...
    const notPending = await api(`/paiements-validation/${payePay.id}/valider`, { method: 'POST', jar });
    ...
    const loyer = await loyerOf(retard.logement_id);
    const val = await api(`/paiements-validation/${retard.id}/valider`, { method: 'POST', jar });
    ...
    // Le mois courant existe déjà (déclaré) : l'échéance du mois courant
    // n'est PAS dupliquée (anti-doublon) par la validation du mois précédent.
    const { data: echsCourant } = await service.from('paiements').select('id, statut').eq('locataire_id', loc1.id).eq('mois', seed.month);
    if (echsCourant.length === 1) r.pass(S, 'validation mois précédent : échéance courante non dupliquée');
    else r.fail(S, 'validation mois précédent : échéance courante non dupliquée', `len=${echsCourant.length}`);

    const again = await api(`/paiements-validation/${retard.id}/valider`, { method: 'POST', jar });
    ...
    const courant = await payOf(loc1.id, seed.month);
    const valCourant = await api(`/paiements-validation/${courant.id}/valider`, { method: 'POST', jar });
    ...
    else if (Number(echeance.montant) !== Number(loyer)) r.fail(S, 'montant de l\'échéance relu en base', `${echeance.montant} != ${loyer}`);
    ...
    const redeclPaye = await api(`/locataire/paiements/${courant.id}/declarer`, { method: 'POST', jar: t1, body: { moyen_paiement_id: moyens[0].id } });
  ...
  await r.section('refus + re-déclaration', async () => {
    ...
    const retard21 = await payOf(loc21.id, seed.prev);
    const dec = await api(`/locataire/paiements/${retard21.id}/declarer`, { method: 'POST', jar: t21, body: { moyen_paiement_id: waveMoyen.id } });
    if (!expectSuccess(r, dec, S, 'déclaration own2loc1')) return;

    const wrongOwner = await api(`/paiements-validation/${retard21.id}/refuser`, { method: 'POST', jar: otherJar, body: { motif: 'Paiement non reçu' } });
    ...
    const noMotif = await api(`/paiements-validation/${retard21.id}/refuser`, { method: 'POST', jar, body: {} });
    ...
    const longMotif = await api(`/paiements-validation/${retard21.id}/refuser`, { method: 'POST', jar, body: { motif: 'x'.repeat(201) } });
    ...
    const rej = await api(`/paiements-validation/${retard21.id}/refuser`, { method: 'POST', jar, body: { motif: 'Paiement non reçu' } });
    ...
    // Pas d'échéance créée par le refus (mois courant non avancé).
    const { data: echs } = await service.from('paiements').select('id').eq('locataire_id', loc21.id).eq('mois', seed.month);
    if (echs.length === 1) r.pass(S, 'refus sans avance d\'échéance');
    else r.fail(S, 'refus sans avance d\'échéance', `len=${echs.length}`);

    const redecl = await api(`/locataire/paiements/${retard21.id}/declarer`, { method: 'POST', jar: t21, body: { moyen_paiement_id: waveMoyen.id, reference: 'TRY-2' } });
  ...
  await r.section('concurrence : doubles requêtes simultanées', async () => {
    ...
    const courant21 = await payOf(loc21.id, seed.month);
    const [a, b] = await Promise.all([
      api(`/locataire/paiements/${courant21.id}/declarer`, { method: 'POST', jar: t21, body: { moyen_paiement_id: moyenId } }),
      api(`/locataire/paiements/${courant21.id}/declarer`, { method: 'POST', jar: t21, body: { moyen_paiement_id: moyenId } }),
    ]);
    ...
    const [v1, v2] = await Promise.all([
      api(`/paiements-validation/${courant21.id}/valider`, { method: 'POST', jar }),
      api(`/paiements-validation/${courant21.id}/valider`, { method: 'POST', jar }),
    ]);
